        ...
        success: function (response) {
            if (response && response.result) {
                ...
                for (key in response.result) {
                    ...
                    tr.append(
                    $('<td>').html(torrent.name + '<br><small><i class="fa fa-long-arrow-down"></i> ' + getReadableFileSizeString(torrent.download_payload_rate) + '/s <i class="fa fa-long-arrow-up"></i> ' + getReadableFileSizeString(torrent.upload_payload_rate) + '/s</small>'),
...
function setConnectToServer(servers) {
    ...
    for (server in servers) {
        ...
        content += '<td style="width:10%;text-align:left;"><input type="radio" name="serverId" data-port=' + servers[server][2] + ' value="' + servers[server][0] + '" style="margin:0;"></td>';
    ...
    var modalButtons = {
        'Connect': function () {
            ...
            $.ajax({
                'url': WEBDIR + 'deluge/daemon/stop/' + $('input[name=serverId]:checked', '#serversForm').attr('data-port'),
                    'success': function (response) {
                    if (response && !response.error) {
            ...
            $.ajax({
                'url': WEBDIR + 'deluge/daemon/start/' + $('input[name=serverId]:checked', '#serversForm').attr('data-port'),
                    'success': function (response) {
                    if (response && !response.error) {
                        notify('Info', 'Successfully started daemon', 'success', 5);
                    } else {
                        notify('Error', 'Problem starting daemon: ' + response.error, 'error', 5);
                    }
                }
            })
        },
