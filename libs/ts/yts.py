...
class YTS(object):
    def __init__(self):
        self.urls = {
                     'search': 'https://yts.to/api/v2/list_movies.json?query_term=%s&sort_by=seeds&limit=50',
                     'movieinfo': 'https://yts.to/api/v2/movie_details.json?movie_id=%s',
                     'suggestion': 'https://yts.to/api/v2/movie_suggestions.json?movie_id=%s',
                     'upcoming': 'https://yts.to/api/v2/list_upcoming.json'
                    }
    ...
    def _fetch(self, url):
        try:
            ...
            self.logger.debug('Failed to return %s %s' % (url, e))
            return []
    ...
    def search(self, q, cat):
        try:
            ...
            self.logger.debug('Error fetching %s cat %s from yts %s' % (q, cat, e))
            return []
